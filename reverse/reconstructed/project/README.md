...
- The frontend runtime implements title/setup/select/practice (`0x02..0x05`), gallery/achievements/enemy encyclopedia/manual (`0x06..0x09`), options/config/key config (`0x0a..0x0d`), replay selection (`0x0e/0x0f`), and ranking states (`0x11/0x12`). Options/Assist/Config/KeyConfig now use the recovered repeat counters, release-gated capture, keyboard/controller scan rules, menu sound multipliers, and separate `save.dat`/`config.dat` persistence. Replay metadata is parsed, but recorded-input playback is still deferred.
...
- The `Ctrl+F6` playable runtime covers Stage 1-4 schedules. Player movement, option formations, normal shots, projectile motion/collision, rewards, HUD panels, and the reviewed Stage 1-4 enemy families follow local Ghidra evidence. Shared enemy type `0x0a` now uses its recovered constructor, 120-frame seeded entry, movement/firing state, multipart draw, gauge, death, and bounds in every stage instead of falling back to generic behavior. Stage 2 scheduled enemies `0x19..0x21` use their recovered constructors, state timers, movement, firing, multipart draw, gauge, death, bounds, child, gate, and `0x2a/0x2b/0x2c/0x37` death-effect logic. Reward items use the original `Item.png` frame/style/visibility rules, collection gates, terminal-frame lifetime, and collection audio. Stage 1 Boss `0x138` has its seven-state, three-HP-bar loop, child emitters, phase breaks, and clear latch; later bosses and the still-unported portions of the generic effect queue remain explicit gaps.
