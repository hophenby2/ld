...
- The `F6` playable stage uses arrow keys to move, `Shift` to focus/slow, `Z` to shoot, `R` to reset the runtime slice, and number keys `1`/`2`/`4` to switch between the Stage 01, Stage 02, and first-pass Stage 04 schedule slices. It now draws an original-coordinate right HUD/sidebar approximation; Stage 04 has an evidence-backed focused behavior pass for reviewed spawn types plus a fuller `Bullet.png` visual-selector map.
...
Controls: arrow keys move, `Shift` focuses/slows movement, `Z` shoots, `R` resets the active runtime slice, and `1`/`2`/`4` switches between the Stage 01 slice, Stage 02 slice, and a sparse Stage 04 slice. The in-stage view draws a right-side HUD aligned to original evidence coordinates, with conservative placeholder score/gauge/token values until reward/player-state reconstruction is complete. Stage 04 now maps more Bullet.png visual selectors and gives reviewed types `0x35`, `0x36`, `0x37`, `0x38`, `0x39/0x3a`, `0x3d`, and `0x3f` distinct approximation patterns. These slices are reconstruction probes designed to make stage logic runnable and inspectable; they are not recovered original source and do not yet implement exact helper-by-helper gameplay.
