...
- The `Ctrl+F6` playable runtime accepts Stage 1-10 (`1`..`9`, `0` for Stage 10). Stage 5-10 use direct transcriptions of their original frame/modulo/gate schedules, the recovered `UNK_140538ad8` lengths, deterministic frame-seeded random coordinates, dedicated background resources, per-stage BGM, and the original Boss handoff families. Stage 9 includes its six mid-stage roots; Stage 10 keeps the `0x14c -> 0x141` transition instead of treating frame 12000 as an ordinary boss spawn. Player movement, option formations, normal shots, projectile motion/collision, rewards, and HUD panels remain shared across all stages. Reviewed Stage 1-4 enemies retain their exact helper ports, while later enemy families now use their recovered constructor/update dispatch, differentiated movement/fire/display paths, gate ownership, and boss phase/clear state rather than the old generic fallback. Some late Boss child patterns, effect nodes, and exact render-layer ordering still require further helper-by-helper recovery.
